...
export const nodeAccessMiddleware = (requiredAction) => async (req, res, next) => {
  try {
    ...
    if (!permission || !permission.accessType.includes(requiredAction)) {
      return next(createError.Forbidden("You do not have permission for this action"));
    }
...
// Department Routes
// --------------------
router.get("/departments",authMiddleware, nodeAccessMiddleware(["view"]) ,  getDepartments);
router.post("/departments", createDepartment);
router.patch("/departments/:id",authMiddleware ,nodeAccessMiddleware(["update"]), updateDepartment);
